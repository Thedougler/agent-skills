...
description: >
  ...
  notes (from an inbox folder or pasted) and produces: a session recap file, world state
  updates, situation file updates, faction clock review, party tracking refresh, and any
  config file updates. Ensures nothing drifts after a session.
disable-model-invocation: true
---

> Sandbox rules are in CLAUDE.md (always loaded). Operational rules: see your project's wiki-init skill references.
...
One of:
- A file in your inbox directory (e.g. `Inbox/Session-04-Notes.md`) — raw session notes
...
1. `<wiki>/hot.md` (or your current world state file)
...
### 3. Update the world state file

Update each section of `<wiki>/hot.md` (or your current world state file):
...
Update any project configuration files that track current arc, active factions, or party state (e.g. CLAUDE.md party/faction blocks) to match the new world state.
